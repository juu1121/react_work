...
function Post(props) {
    ...
    const move = (pageNum)=>{
        ...
        //navigate(`/posts?pageNum=${pageNum}&condition=${condition}&keyword=${keyword}`); //이전꺼?
    }
    ...
    return (
        <>
            ...
            <select name="condition" id="search" onChange={handleSearchChange} 
            ...
            <input type="text" placeholder='검색어...' name="keyword" onChange={handleSearchChange} 
                value={searchState.keyword}/>
            <button onClick={()=>move(1)}>검색</button>
            <button onClick={handleReset}>Reset</button>
            { pageInfo.keyword && <p> <strong>{pageInfo.totalRow}</strong> 개의 글이 검색 되었습니다 </p>}
        </>
    );
}
